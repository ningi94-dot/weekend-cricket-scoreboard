...
export async function POST(request: Request, context: { params: Promise<{ matchId: string }> }) {
  try {
    ...
    if (!body.strikerId || (!allowNoNonStriker && !nonStrikerId) || !body.bowlerId || !body.wicketKeeperId || !body.umpireId) {
      return NextResponse.json({ message: allowNoNonStriker ? "Choose second-innings striker, bowler, wicket keeper, and umpire. Non-striker is optional." : "Choose second-innings openers, bowler, wicket keeper, and umpire." }, { status: 400 });
    }
