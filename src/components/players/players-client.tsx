...
export function PlayersClient() {
  ...
  async function loadPlayers() {
    try {
      ...
      setPlayers(rows.map((row) => {
        ...
        return { id: row.id, name: row.name, battingStyle: battingFromDb[row.batting_style], bowlingStyle: bowlingFromDb[row.bowling_style], matches: stats.matches, runs: stats.runs, highestScore: stats.highest.runs, wickets: stats.wickets };
      }));
    } catch (error) {
  ...
  async function submitPlayer(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const supabase = getSupabaseBrowserClient();
      const record = { name, batting_style: battingToDb[form.battingStyle], bowling_style: bowlingToDb[form.bowlingStyle] };
  ...
  return (
    <>
              ...
              <div className="flex items-start justify-between gap-3">
                <Link href={`/players/${player.id}`} className="flex min-w-0 flex-1 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-[var(--muted)]">{player.battingStyle} - {player.bowlingStyle}</p>
                  </div>
                </Link>
                <div className="flex gap-1">
                  <button onClick={() => { setEditingId(player.id); setForm({ name: player.name, battingStyle: player.battingStyle, bowlingStyle: player.bowlingStyle }); setIsFormOpen(true); }} className="rounded-lg px-2 py-1 text-xs font-semibold text-[var(--brand)]">Edit</button>
      ...
      {isFormOpen && <div className="fixed inset-0 z-30 flex items-end bg-black/35 sm:items-center sm:justify-center sm:p-4"><form onSubmit={(event) => void submitPlayer(event)} className="w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-3xl"><div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-bold">{editingId ? "Edit player" : "Add player"}</h2><button type="button" onClick={() => setIsFormOpen(false)} className="p-2 text-[var(--muted)]">Close</button></div><label className="block text-sm font-semibold">Player name<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="mt-1.5 min-h-11 w-full rounded-lg border border-[var(--line)] px-3 font-normal" /></label><Select label="Batting style" value={form.battingStyle} options={battingStyles} onChange={(value) => setForm({ ...form, battingStyle: value as BattingStyle })} /><Select label="Bowling style" value={form.bowlingStyle} options={bowlingStyles} onChange={(value) => setForm({ ...form, bowlingStyle: value as BowlingStyle })} /><button className="mt-6 min-h-11 w-full rounded-lg bg-[var(--brand)] text-sm font-bold text-white">Save player</button></form></div>}
    </>
  );
}

function Select({ label, value, options, onChange }: { label: string; value: string; options: readonly string[]; onChange: (value: string) => void }) {
  return <label className="mt-4 block text-sm font-semibold">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 min-h-11 w-full rounded-lg border border-[var(--line)] bg-white px-3 font-normal">{options.map((option) => <option key={option}>{option}</option>)}</select></label>;
}
