...
export function PlayerProfileClient({ playerId }: { playerId: string }) {
  ...
  const playedMatchIds = new Set(innings.filter((item) => deliveries.some((delivery) => delivery.innings_id === item.id && (delivery.striker_id === playerId || delivery.non_striker_id === playerId || delivery.bowler_id === playerId || delivery.fielder_id === playerId || delivery.catch_drop_fielder_id === playerId))).map((item) => item.match_id));
...
function ColorCard({ title, stats, tone }: { title: string; stats: [string, string | number][]; tone: string }) {
  return <div className={`rounded-lg p-4 text-center ${tone}`}><p className="text-xs font-bold uppercase tracking-[0.12em] text-stone-700">{title}</p><div className="mt-3 space-y-2">{stats.map(([label, value]) => <div key={label}><p className="text-xl font-black leading-none">{value}</p><p className="text-[11px] font-semibold text-stone-700">{label}</p></div>)}</div></div>;
}
