...
import { formatOversLabel, formatTournamentDate, tournamentLeaders, type CapTone, type PerformanceRow, type SquadRow, type TournamentRow } from "@/lib/cricket/tournament-stats";
import { getSupabaseBrowserClient } from "@/lib/supabase/client";

type LeaderRow = PerformanceRow & { tone?: CapTone };

export function TournamentLeadersClient({ tournamentId }: { tournamentId: string }) {
  ...
  const sections = [
    ...
    {
      title: "Most catches",
      rows: leaders.catches.map((row, index) => ({ id: row.playerId, rank: index + 1, name: row.name, value: `${row.catches} catch${row.catches === 1 ? "" : "es"}`, detail: "Fielding", isLeader: index === 0 })),
    },
  ];

  if (isLoading) return <p className="text-sm text-[var(--muted)]">Loading tournament leaders...</p>;
  ...
  return (
    <section className="space-y-4">
      ...
      <header className="rounded-lg bg-white p-4 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--brand)]">Tournament leaders</p>
      ...
      <div className="grid gap-3">
        {sections.map((section) => <ExpandableLeaderSection key={section.title} title={section.title} rows={section.rows} />)}
      </div>
    </section>
  );
}
  ...
  return (
    <section className="rounded-lg border border-[var(--line)] bg-white p-4">
      ...
      <ul className="mt-3 space-y-2 text-sm">
        {visibleRows.length ? visibleRows.map((row) => <LeaderItem key={row.id} row={row} />) : <li className="rounded-lg bg-stone-50 p-3 text-[var(--muted)]">No data yet.</li>}
      </ul>
    </section>
  );
}
  ...
  return (
    <li className={`flex items-center justify-between gap-3 rounded-lg p-3 ${row.isLeader ? "bg-amber-50 ring-2 ring-amber-200" : "bg-stone-50"}`}>
      <span className="flex min-w-0 items-center gap-2">
        <span className="shrink-0 font-black">{row.rank}.</span>
        <span className="min-w-0">
          <span className="block truncate font-black">
            {row.name}
            {row.tone && <CapIcon tone={row.tone} />}
          </span>
...
function CapIcon({ tone }: { tone: CapTone }) {
  return (
    <Image
      src={tone === "orange" ? "/images/cap-orange.png" : "/images/cap-purple.png"}
      alt=""
      aria-hidden="true"
      width={28}
      height={22}
      className="ml-1 inline h-5 w-6 object-contain align-middle"
    />
  );
}
