...
type MatchCard = CricketMatch & { startTime: string | null; isTest: boolean; singleBatterMode: boolean };
...
function defaultMatchForm(): MatchForm {
  return {
    homeTeam: "Team Mahith",
    awayTeam: "Team Rajeev",
    date: nextSundayDate(),
    startTime: "",
...
export function MatchesClient() {
  ...
  const [form, setForm] = useState<MatchForm>(() => defaultMatchForm());
  ...
  async function loadMatches() {
    try {
      ...
      setMatches((data ?? []).map((row) => ({
        ...
        singleBatterMode: row.single_batter_mode,
      })));
    } catch (error) {
  ...
  async function submitMatch(event: FormEvent<HTMLFormElement>) {
    ...
    if (!form.homeTeam.trim() || !form.awayTeam.trim() || !form.date || !form.location.trim()) return;
    try {
      const { error } = await getSupabaseBrowserClient().from("matches").insert({
        ...
        single_batter_mode: form.singleBatterMode,
      });
      if (error) throw error;
  ...
  return (
    <>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-30 flex items-end bg-black/35 sm:items-center sm:justify-center sm:p-4">
          <form onSubmit={(event) => void submitMatch(event)} className="w-full rounded-t-3xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-3xl">
            <div className="mb-5 flex items-center justify-between">
              <h2 className="text-lg font-bold">Create match</h2>
              <button type="button" onClick={() => setIsFormOpen(false)} className="p-2 text-[var(--muted)]">Close</button>
            </div>
            <Field label="Team A" value={form.homeTeam} onChange={(value) => setForm({ ...form, homeTeam: value })} />
            <Field label="Team B" value={form.awayTeam} onChange={(value) => setForm({ ...form, awayTeam: value })} />
            <label className="mt-4 block text-sm font-semibold">Tournament<select value={form.tournamentId} onChange={(event) => setForm({ ...form, tournamentId: event.target.value })} className="mt-1.5 min-h-11 w-full rounded-lg border border-[var(--line)] bg-white px-3 font-normal"><option value="">No tournament</option>{tournaments.map((tournament) => <option key={tournament.id} value={tournament.id}>{tournament.name}</option>)}</select></label>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Field label="Match date" type="date" value={form.date} onChange={(value) => setForm({ ...form, date: value })} />
              <Field label="Start time" type="time" value={form.startTime} onChange={(value) => setForm({ ...form, startTime: value })} required={false} />
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Field label="Overs" type="number" value={String(form.overs)} onChange={(value) => setForm({ ...form, overs: Number(value) || 0 })} />
              <label className="flex items-center gap-2 pt-7 text-sm font-semibold"><input type="checkbox" checked={form.isTest} onChange={(event) => setForm({ ...form, isTest: event.target.checked })} /> Test/dummy</label>
            </div>
